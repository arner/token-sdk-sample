...
git --version
go version
tokengen version || { echo "install tokengen:

go install github.com/hyperledger-labs/fabric-token-sdk/cmd/tokengen@010bf3c6d930"; exit 1; }
...
while ! fabric-ca-client getcainfo -u localhost:27054; do sleep 1; done
