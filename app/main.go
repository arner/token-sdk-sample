...
func main() {
	...
	tokenAPI := TokenAPI{
		AuditorAPI:    routes.AuditorAPI{Auditor: auditor.AuditorService{FSC: fsc}},
		IssuerAPI:     routes.IssuerAPI{Issuer: issuer.IssuerService{FSC: fsc}},
		OwnerAPI:      routes.OwnerAPI{Owner: owner.OwnerService{FSC: fsc}},
		OperationsAPI: routes.OperationsAPI{},
	}
	port := getEnv("PORT", "9000")
	err := routes.StartWebServer(port, tokenAPI, logger)
	if err != nil {
...
func startFabricSmartClient(confDir string) Node {
	...
	// Auditor
	succeedOrPanic(registry.RegisterResponder(&auditor.AuditView{}, &ttx.AuditingViewInitiator{}))
	// Owner
	succeedOrPanic(registry.RegisterResponder(&owner.AcceptCashView{}, "github.com/hyperledger/fabric-samples/token-sdk/app/issuer/IssueCashView"))
	succeedOrPanic(registry.RegisterResponder(&owner.AcceptCashView{}, &owner.TransferView{}))
