...
func (a OwnerAPI) Transfer(ctx context.Context, request TransferRequestObject) (TransferResponseObject, error) {
	...
	txID, err := a.Owner.TransferTokens(code, value, sender, recipient, recipientNode, message)
	if err != nil {
...
// (GET /owner/accounts)
func (a OwnerAPI) OwnerAccounts(ctx context.Context, request OwnerAccountsRequestObject) (OwnerAccountsResponseObject, error) {
	balances, err := a.Owner.GetAllBalances()
	if err != nil {
...
func (a OwnerAPI) OwnerAccount(ctx context.Context, request OwnerAccountRequestObject) (OwnerAccountResponseObject, error) {
	var code string
	if request.Params.Code != nil {
		code = *request.Params.Code
	}
	balance, err := a.Owner.GetBalance(request.Id, code)
	if err != nil {
...
func (a OwnerAPI) OwnerTransactions(ctx context.Context, request OwnerTransactionsRequestObject) (OwnerTransactionsResponseObject, error) {
	var history []owner.TransactionHistoryItem
	var err error

	history, err = a.Owner.GetHistory(request.Id)
	if err != nil {
...
func (a OwnerAPI) Redeem(ctx context.Context, request RedeemRequestObject) (RedeemResponseObject, error) {
	code := request.Body.Amount.Code
	value := uint64(request.Body.Amount.Value)
	account := request.Id
	var message string
	...
	txID, err := a.Owner.RedeemTokens(code, value, account, message)
	if err != nil {
