...
type AuditorAPI struct {
	Auditor auditor.AuditorService
}

// Get an account and their balance of a certain type
// (GET /auditor/accounts/{id})
func (a AuditorAPI) AuditorAccount(ctx context.Context, request AuditorAccountRequestObject) (AuditorAccountResponseObject, error) {
	...
	balance, err := a.Auditor.GetBalance(request.Id, *request.Params.Code)
	if err != nil {
		return AuditorAccountdefaultJSONResponse{
			Body: Error{
...
func (a AuditorAPI) AuditorTransactions(ctx context.Context, request AuditorTransactionsRequestObject) (AuditorTransactionsResponseObject, error) {
	var history []auditor.TransactionHistoryItem
	var err error

	history, err = a.Auditor.GetHistory(request.Id)
	if err != nil {
