...
type IssuerAPI struct {
	Issuer issuer.IssuerService
}

// Issue tokens to an account
// (POST /issue)
func (a IssuerAPI) Issue(ctx context.Context, request IssueRequestObject) (IssueResponseObject, error) {
	...
	txID, err := a.Issuer.Issue(code, value, recipient, recipientNode, message)
	if err != nil {
